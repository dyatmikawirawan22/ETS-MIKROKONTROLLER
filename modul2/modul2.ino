...
const char* ssid = "002"; // Enter your WiFi name
const char* password =  "dyatmikawirawan"; // Enter WiFi password
...
//    if (client.connect("ESP32Client", mqttUser, mqttPassword )) {
     if (client.connect("ESP32_Dyatmika")) { 
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == '1') {
    digitalWrite(ledPin, HIGH);   // 🔴 Tambahan baris ini
    MotorOn();
    Serial.println("Motor On + LED On");
  } 
  
  if ((char)payload[0] == '0') {
    digitalWrite(ledPin, LOW);    // 🔴 Tambahan baris ini
    MotorOff(); 
